...
var debug = require('debug')('loopback:connector:disco-rest');
var RestResource = require('./rest-model');
var RequestBuilder = require('./rest-builder');
var request = require('request');
var discoveryAgent = require('discovery-agent');
...
function initializeDataSource(dataSource, callback) {
  ...
  var connector = new DiscoRestConnector(baseURL, settings);
  ...
  if(settings.serviceDiscovery.pool) {
    connector.createPool(settings.serviceDiscovery.pool.type);
  }

  if(settings.serviceDiscovery.registration) {
    debug('Registration with discovery service: %O', settings.serviceDiscovery.registration);
    connector.register(
      settings.serviceDiscovery.registration.type,
      settings.serviceDiscovery.registration.advertiseHost,
      settings.serviceDiscovery.registration.advertisePort,
      (err, registrationDetails) => {
        if (err) {
          debug('Registration with discovery service failed: %O', err);
          return callback(err);
        }

        debug('Registered with discovery service: %O', registrationDetails);
        return callback();
      }
    );
  }
  else {
    debug('Skipping registration with discovery service...');
    callback();
  }
};

/**
 * @class DiscoRestConnector
*/
exports.DiscoRestConnector = DiscoRestConnector;
 ...
 */
function DiscoRestConnector(baseURL, settings) {
  settings = settings || {};
  var options = settings.options || {};
  var defaults = settings.defaults || {
    serviceDiscovery: {
      enabled: true,
      adapter: discoveryAgent.ADAPTER_TYPES.CONSUL,
      providers: [{ host: '127.0.0.1' }]
    }
  };
  ...
  this.agentPool = function () {};

  debug('DiscoRestConnector:settings', settings);
  ...
  debug('DiscoRestConnector:this._settings', this._settings);
  this._request = request.defaults(this._settings);

  // @todo Support an array of providers
  this._discoveryAgent = new discoveryAgent.DiscoveryAgent(
    this._settings.serviceDiscovery.adapter,
    this._settings.serviceDiscovery.providers[0]
  );
}

/**
 * Register the service with the service discovery provider
 * @param {string} type
 * @param {string} host
 * @param {number} port
 * @param {function} cb
 */
DiscoRestConnector.prototype.register = function(type, host, port, cb) {
  this._discoveryAgent.register(type, host, port)
    .then(registrationDetails => {
      cb(null, registrationDetails);
    })
    .catch(err => {
      cb(err);
    });
 ...
 * @param {string} type
 */
DiscoRestConnector.prototype.createPool = function(type) {
  this.agentPool = this._discoveryAgent.createPool(type);
};
 ...
 * @param {function} cb
 */
DiscoRestConnector.prototype.getURLFromPool = function(url, cb) {
  this.agentPool
    .then(service => {
      cb(null, service.toURI(url));
    })
    .catch(cb);
}
 ...
 */
DiscoRestConnector.prototype.define = function defineModel(definition) {
 ...
 * @param {object} definition The model description
 */
DiscoRestConnector.prototype.installPostProcessor = function installPostProcessor(definition) {
  var dates = [];
 ...
 * @returns {{}}
 */
DiscoRestConnector.prototype.preProcess = function preProcess(data) {
  var result = {};
 ...
 */
DiscoRestConnector.prototype.postProcess = function postProcess(model, data, many) {
  var result = data;
 ...
 * @returns {*}
 */
DiscoRestConnector.prototype.getResource = function getResourceUrl(model) {
 ...
 * @param {function} [callback] The callback function
 */
DiscoRestConnector.prototype.create = function create(model, data, callback) {
 ...
 * @param {function} [callback] The callback function
 * */
DiscoRestConnector.prototype.updateOrCreate = function(model, data, callback) {
  var self = this;
 ...
 * @returns {function}
 */
DiscoRestConnector.prototype.responseHandler = function(model, callback, many) {
  var self = this;
 ...
 * @param {function} [callback] The callback function
 */
DiscoRestConnector.prototype.save = function save(model, data, callback) {
 ...
 * @param {function} [callback] The callback function
 */
DiscoRestConnector.prototype.exists = function exists(model, id, callback) {
 ...
 * @param {function} [callback] The callback function
 */
DiscoRestConnector.prototype.find = function find(model, id, callback) {
 ...
 * @param {function} [callback] The callback function
 */
DiscoRestConnector.prototype.destroy = function destroy(model, id, callback) {
 ...
 * @param {function} [callback] The callback function
 */
DiscoRestConnector.prototype.all = function all(model, filter, callback) {
 ...
 * @param {function} [callback] The callback function
 */
DiscoRestConnector.prototype.destroyAll = function destroyAll(model, where, callback) {
  if (where && where.id) {
 ...
 */
DiscoRestConnector.prototype.count = function count(model, callback, where) {
 ...
 * @param {function} [callback] The callback function
 */
DiscoRestConnector.prototype.updateAttributes = function(model, id, data, callback) {
  data.id = id;
 ...
 */
DiscoRestConnector.prototype.getTypes = function() {
  return ['rest'];
};
